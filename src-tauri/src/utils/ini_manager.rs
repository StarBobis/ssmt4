...
impl IniManager {
    ...
    pub fn set(&mut self, section: &str, key: &str, value: &str) {
        ...
        for (i, line) in lines.iter_mut().enumerate() {
            ...
            if clean_line.starts_with('[') && clean_line.ends_with(']') {
                ...
                println!("[IniManager] Found section: [{}] at line {}", current_section, i);

                if current_section == section_lower {
                    println!("[IniManager] Entering target section: [{}]", current_section);
                ...
                if let Some(eq_idx) = line.find('=') {
                    let current_key = line[..eq_idx].trim();
                     if current_key.eq_ignore_ascii_case(key) {
                        println!("[IniManager] Found key '{}' at line {}. Updating value.", current_key, i);
        ...
        if !key_found {
            ...
            if target_section_index != -1 {
                ...
                let insert_pos = if last_line_of_section != -1 {
                    ...
                    for (i, line) in lines.iter().enumerate().skip((target_section_index + 1) as usize) {
                         let trimmed = line.trim();
                         let clean_line = trimmed.split(';').next().unwrap_or("").trim();
                ...
                println!("[IniManager] Section [{}] not found. Creating at EOF.", section);
    ...
    pub fn save(&self) -> Result<(), String> {
        ...
        println!("[IniManager] Successfully wrote {} bytes.", self.content.len());
        Ok(())
    }
}
